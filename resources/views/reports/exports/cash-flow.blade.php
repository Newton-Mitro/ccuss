...
<body>
    ...
    <div class="report-heading">
        <div class="eyebrow">General Accounting · Cash Flow Statement</div>
    ...
    @php
        ...
        $grandTotal = collect($rows)->sum(fn ($row) => (float) ($row[2] ?? 0));
        $money = fn ($amount) => ((float) $amount < 0 ? '-BDT ' : 'BDT ') . number_format(abs((float) $amount), 2);
    @endphp

    <table>
        <thead>
            <tr class="page-gap"><th colspan="3"></th></tr>
            <tr><th>Period</th><th>Category</th><th class="number">Net Cash</th></tr>
        </thead>
        @forelse($groupedRows as $category => $categoryRows)
            @php $categoryTotal = $categoryRows->sum(fn ($row) => (float) ($row[2] ?? 0)); @endphp
            <tbody>
                <tr class="group"><td colspan="3">{{ $category }} Activities</td></tr>
                @foreach($categoryRows as $row)
                    <tr class="account"><td>{{ $row[1] ?: '—' }}</td><td>{{ $row[0] }}</td><td class="number {{ (float) $row[2] < 0 ? 'negative' : '' }}">{{ $money($row[2]) }}</td></tr>
                @endforeach
                <tr class="total"><td colspan="2" class="number">Net Cash from {{ $category }}</td><td class="number">{{ $money($categoryTotal) }}</td></tr>
            </tbody>
        @empty
            <tbody><tr><td colspan="3">No cash flow records found.</td></tr></tbody>
        @endforelse
        <tfoot><tr class="grand-total"><td colspan="2" class="number">Net Increase / Decrease in Cash</td><td class="number">{{ $money($grandTotal) }}</td></tr></tfoot>
    </table>

    <div class="footer">{{ $organizationFooter ? $organizationFooter . ' · ' : '' }}Cash Flow Statement · Generated {{ $generatedAt }}</div>
