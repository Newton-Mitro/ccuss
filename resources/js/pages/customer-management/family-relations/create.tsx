...
interface CreateFamilyRelationProps {
    customers: Customer[];
}

export default function Create({ customers }: CreateFamilyRelationProps) {
        ...
        <CustomAuthLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="animate-in space-y-8 px-4 py-6 text-foreground fade-in">
                <HeadingSmall
                    ...
                    description="Link a customer with a family member and define their relationship."
                />
                        ...
                        {customerSuggestions.length > 0 && (
                            <ul className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md border border-border bg-background shadow-lg">
                                {customerSuggestions.map((c, idx) => {
                                    const start = c.name
                                        .toLowerCase()
                                        .indexOf(customerSearch.toLowerCase());
                                    const end = start + customerSearch.length;
                                    return (
                                        <li
                                            key={c.id}
                        ...
                        {relativeSuggestions.length > 0 ? (
                            <ul className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md border border-border bg-background shadow-lg">
                                {relativeSuggestions.map((c, idx) => {
                                    const start = c.name
                                        .toLowerCase()
                                        .indexOf(relativeSearch.toLowerCase());
                                    const end = start + relativeSearch.length;
                                    return (
                                        <li
                                            key={c.id}
                                            className={`cursor-pointer px-3 py-2 hover:bg-gray-100 ${
                                                idx === activeRelativeIndex
                                                    ? 'bg-gray-200'
                                                    : ''
                                            }`}
                                            onClick={() => {
                            ...
                            <p className="absolute z-10 mt-1 w-full px-3 py-2 text-sm text-gray-500">
                                No matches found
                            </p>
                        ) : null}
                    </div>
