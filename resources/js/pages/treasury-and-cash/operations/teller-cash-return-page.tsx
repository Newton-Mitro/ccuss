...
export default function VaultToVaultTransferPage() {
    ...
    const syncVoucher = (total: number, denomsData: Denom[]) => {
        if (!fromTeller || !toVault || total <= 0) return;

        const lines = [
            {
                subledger_id: fromTeller.id,
                name: fromTeller.name,
                debit: total,
                credit: 0,
            },
            {
        ...
        setData(
            'narration',
            `Cash deposited from vault ${fromTeller.name} to bank ${toVault.name}`,
        );
    };
    ...
    const submitTransfer = () => {
        ...
        if (!fromTeller || !toVault) {
            alert('Select vaults');
            return;
        }

        if (fromTeller.id === toVault.id) {
