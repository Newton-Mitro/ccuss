...
The Deposit Module handles all deposit products (savings, recurring, term deposits, shares) and their associated accounts, transactions, cheques, interest accrual, penalties, and statements. Below is a step-by-step action flow.
...
2. **Open Deposit Account**
    ...
    - **Outcome:** Account is ready for deposits, withdrawals, and transactions.
...
8. **Penalty Assessment**
    ...
    - **Outcome:** Penalties are recorded and may generate transactions.

9. **Account Fees**
    - **Purpose:** Apply fees like opening, maintenance, or transaction charges.
    - **Key Fields:** `fee_type`, `amount`, `frequency`, `is_paid`.
    - **Outcome:** Fees are tracked and can be linked to transactions.

10. **Account Statement**
    - **Purpose:** Generate statement entries from all transactions.
    - **Key Fields:** `debit`, `credit`, `balance`, `posted_at`.
    - **Outcome:** Complete ledger for account with balances and transactions.
